...
class ENGINE_DLL CShaderAddEffect final : public CShader
{
private:
	explicit CShaderAddEffect(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	explicit CShaderAddEffect(const CShaderAddEffect& rhs);
	virtual ~CShaderAddEffect() = default;
...
public:
	...
	static CShaderAddEffect*	Create(ID3D12Device* pGraphicDebice, ID3D12GraphicsCommandList* pCommandList);
private:
	virtual void			Free();
