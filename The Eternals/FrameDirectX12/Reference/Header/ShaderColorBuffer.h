...
class ENGINE_DLL CShaderColorBuffer : public CShader
{
private:
	explicit CShaderColorBuffer(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	explicit CShaderColorBuffer(const CShaderColorBuffer& rhs);
	virtual ~CShaderColorBuffer() = default;
...
public:
	...
	static CShaderColorBuffer*	Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
private:
	virtual void				Free();
};
