...
class ENGINE_DLL CShaderBlur final : public CShader
{
private:
	explicit CShaderBlur(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	explicit CShaderBlur(const CShaderBlur& rhs);
	virtual ~CShaderBlur() = default;
...
public:
	...
	static CShaderBlur*	Create(ID3D12Device* pGraphicDebice, ID3D12GraphicsCommandList* pCommandList);
private:
	virtual void			Free();
