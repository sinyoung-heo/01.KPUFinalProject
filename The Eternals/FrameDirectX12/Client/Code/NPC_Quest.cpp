...
_int CNPC_Quest::Update_GameObject(const _float& fTimeDelta)
{
	...
	if (m_bIsReturn)
	{
		Return_Instance(CInstancePoolMgr::Get_Instance()->Get_NPCQuestPool(), m_uiInstanceIdx);
...
CNPC_Quest** CNPC_Quest::Create_InstancePool(ID3D12Device* pGraphicDevice, 
											 ID3D12GraphicsCommandList* pCommandList, 
											 wstring wstrMeshTag, 
											 const _uint& uiInstanceCnt)
{
	CNPC_Quest** ppInstance = new (CNPC_Quest * [uiInstanceCnt]);

	for (_uint i = 0; i < uiInstanceCnt; ++i)
	{
		ppInstance[i] = new CNPC_Quest(pGraphicDevice, pCommandList);
