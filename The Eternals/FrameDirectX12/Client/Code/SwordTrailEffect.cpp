...
CSwordTrailEffect::CSwordTrailEffect(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	: Engine::CGameObject(pGraphicDevice, pCommandList)
{
}


HRESULT CSwordTrailEffect::Ready_GameObject(wstring wstrMeshTag,
											 const _vec3 & vScale,
											 const _vec3 & vAngle, 
...
HRESULT CSwordTrailEffect::LateInit_GameObject()
{
...
_int CSwordTrailEffect::Update_GameObject(const _float & fTimeDelta)
{
...
_int CSwordTrailEffect::LateUpdate_GameObject(const _float & fTimeDelta)
{
...
void CSwordTrailEffect::Render_GameObject(const _float& fTimeDelta)
{
	Set_ConstantTable();
	m_pMeshCom->Render_MagicCircleMesh(m_pShaderCom, m_pDescriptorHeaps,0,27, 16,17,18); //D  N S Sha Dis
}
void CSwordTrailEffect::Render_CrossFilterGameObject(const _float& fTimeDelta)
{
	Set_ConstantTable();
	m_pMeshCom->Render_StaticMesh(m_pCrossFilterShaderCom);
}
HRESULT CSwordTrailEffect::Add_Component(wstring wstrMeshTag)
{
...
void CSwordTrailEffect::Set_ConstantTable()
{
...
HRESULT CSwordTrailEffect::SetUp_DescriptorHeap(vector<ComPtr<ID3D12Resource>> vecTexture, vector<ComPtr<ID3D12Resource>> vecShadowDepth)
{
...
Engine::CGameObject* CSwordTrailEffect::Create(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList,
												wstring wstrMeshTag, 
												const _vec3 & vScale,
												const _vec3 & vAngle,
												const _vec3 & vPos, const _vec3& vDir, const float& vAngleOffset)
{
	CSwordTrailEffect* pInstance = new CSwordTrailEffect(pGraphicDevice, pCommandList);
