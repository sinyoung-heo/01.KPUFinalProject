...
CInventoryButtonClose::CInventoryButtonClose(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList)
	: CGameUIChild(pGraphicDevice, pCommandList)
{
}

HRESULT CInventoryButtonClose::Ready_GameObject(wstring wstrRootObjectTag, 
...
HRESULT CInventoryButtonClose::LateInit_GameObject()
{
...
_int CInventoryButtonClose::Update_GameObject(const _float& fTimeDelta)
{
...
_int CInventoryButtonClose::LateUpdate_GameObject(const _float& fTimeDelta)
{
	if (!m_bIsActive)
		return NO_EVENT;
	...
	if (CMouseCursorMgr::Get_Instance()->Check_CursorInRect(m_tRect) &&
		...
		m_bIsKeyPressing)
	{
		static_cast<CMainMenuInventory*>(m_pObjectMgr->Get_GameObject(L"Layer_UI", L"OptionInventoryNormal"))->Set_IsActiveCanvas(false);
	}
...
void CInventoryButtonClose::Render_GameObject(const _float& fTimeDelta)
{
	CGameUIChild::Render_GameObject(fTimeDelta);
}

HRESULT CInventoryButtonClose::SetUp_MainMenuState(wstring wstrTag, const UI_CHILD_STATE& tState)
{
...
Engine::CGameObject* CInventoryButtonClose::Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList,
												   ...
												   const _long& iUIDepth)
{
	CInventoryButtonClose* pInstance = new CInventoryButtonClose(pGraphicDevice, pCommandList);
...
void CInventoryButtonClose::Free()
{
	CGameUIChild::Free();
