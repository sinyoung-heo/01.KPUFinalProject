...
void CStoreBuyListSlot::Set_CurItemInfo(const char& chItemType, const char& chItemName, const _int& iCnt)
{
...
void CStoreBuyListSlot::KeyInput_MouseButton(const _float& fTimeDelta)
{
			...
			NO_ITEM != m_tCurItemInfo.chItemName)
		{
			if (ItemType_Potion == m_tCurItemInfo.chItemType)
			{
				...
				if (m_uiCnt <= 0)
				{
					m_tCurItemInfo.chItemType = NO_ITEM;
					m_tCurItemInfo.chItemName = NO_ITEM;
					CStoreMgr::Get_Instance()->Min_StoreBuySlotSize();
				}
			}
			else
			{
			...
			CStoreMgr::Get_Instance()->Add_BuyItemSumGold(-m_iPrice);
		}
...
void CStoreBuyListSlot::SetUp_ItemIcon()
{
	...
	case ItemType_Potion:
	{
		...
		{
		case Potion_HP:
			m_iPrice = Item_Potion_HP_COST;
			m_uiTexIdx = 0;
			break;
		case Potion_MP:
			m_iPrice = Item_Potion_MP_COST;
			m_uiTexIdx = 1;
			break;
		}
	}
...
void CStoreBuyListSlot::SetUp_FontPotionCnt(const _float& fTimeDelta)
{
	if (nullptr != m_pFont)
	{
