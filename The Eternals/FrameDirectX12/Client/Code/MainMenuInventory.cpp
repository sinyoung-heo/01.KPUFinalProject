...
_int CMainMenuInventory::LateUpdate_GameObject(const _float& fTimeDelta)
{
	...
	// Active Canvas UI
	CInventoryEquipmentMgr::Get_Instance()->Get_InventoryCanvasClass()->Set_IsActive(m_bIsActiveCanvas);
	CInventoryEquipmentMgr::Get_Instance()->Get_InventoryCanvasClass()->SetUp_ActiveChildUI(m_bIsActiveCanvas);
...
HRESULT CMainMenuInventory::SetUp_MainMenuState()
{
	// IMG RED 
	m_mapMainMenuState[L"Red"] = UI_ROOT_STATE();

	{
		wifstream fin { L"../../Bin/ToolData/2DUIMainMenuInventory_Red.2DUI" };
		if (fin.fail())
			return E_FAIL;
	...
	{
		wifstream fin { L"../../Bin/ToolData/2DUIMainMenuInventory_Blue.2DUI" };
		if (fin.fail())
			return E_FAIL;
