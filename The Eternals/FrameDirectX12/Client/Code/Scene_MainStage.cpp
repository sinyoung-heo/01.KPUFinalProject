...
HRESULT CScene_MainStage::Ready_Scene()
{
	...
	// 泅犁 阁胶磐 拿傅捞 登绢乐瘤 臼栏骨肺 林籍贸府 秦初澜. 昏力 X
...
HRESULT CScene_MainStage::Ready_LayerUI(wstring wstrLayerTag)
{
	...
	Engine::FAILED_CHECK_RETURN(SetUp_UIQuestRequestCanvas(), E_FAIL);
	Engine::FAILED_CHECK_RETURN(SetUp_UIQuesetMiniCanvas(), E_FAIL);
	Engine::FAILED_CHECK_RETURN(SetUp_UIQuestMessageCanvas(), E_FAIL);
...
HRESULT CScene_MainStage::SetUp_UIQuickSlot()
{
	...
	// ItemSwapSlot
	{
		...
		while (true)
		{
			...
			// UIRoot 积己.
			Engine::CGameObject* pRootUI = nullptr;
			pRootUI = CQuickSlotSwapSlot::Create(m_pGraphicDevice, m_pCommandList,
												 wstrRootObjectTag,
												 wstrDataFilePath,
												 vPos,
												 vScale,
												 ...
												 UIDepth);
			m_pObjectMgr->Add_GameObject(L"Layer_UI", wstrRootObjectTag, pRootUI);
			CQuickSlotMgr::Get_Instance()->Set_QuickSlotSwapSlot(static_cast<CQuickSlotSwapSlot*>(pRootUI));
		}
	}
...
HRESULT CScene_MainStage::SetUp_UIQuestRequestCanvas()
{
	// MainMenuSettingCanvas
	CQuestButtonClose* pButtonXMouseNormal  = nullptr;
	CQuestButtonClose* pButtonXMouseOn      = nullptr;
	CQuestButtonClose* pButtonXMouseClicked = nullptr;

	{
		wifstream fin{ L"../../Bin/ToolData/2DUIQuestCanvas.2DUI" };
		if (fin.fail())
			return E_FAIL;
		...
		while (true)
		{
			...
			// UIRoot 积己.
			Engine::CGameObject* pRootUI = nullptr;
			pRootUI = CQuestRequestCanvas::Create(m_pGraphicDevice, m_pCommandList,
												  wstrRootObjectTag,
												  wstrDataFilePath,
												  vPos,
												  vScale,
												  ...
												  UIDepth);
			m_pObjectMgr->Add_GameObject(L"Layer_UI", wstrRootObjectTag, pRootUI);
			CQuestMgr::Get_Instance()->Set_CanvasClass(static_cast<CQuestRequestCanvas*>(pRootUI));
			...
			for (_int i = 0; i < iChildUISize; ++i)
			{
				...
				if (L"UIQuestButton" == vecObjectTag[i])
				{
					pChildUI = CQuestButtonAccept::Create(m_pGraphicDevice, m_pCommandList,
														  wstrRootObjectTag,				// RootObjectTag
														  vecObjectTag[i],					// ObjectTag
														  vecDataFilePath[i],				// DataFilePath
														  vecPos[i],						// Pos
														  ...
														  vecRectScale[i],					// RectScaleOffset
														  UIDepth - 1);					// UI Depth
					CQuestMgr::Get_Instance()->Set_ButtonAccpetClass(static_cast<CQuestButtonAccept*>(pChildUI));
				}
				else if (L"QuestButtonCloseNormal" == vecObjectTag[i] ||
						 L"QuestButtonCloseMouseOn" == vecObjectTag[i] ||
						 L"QuestButtonCloseMouseClickend" == vecObjectTag[i])
				{
					pChildUI = CQuestButtonClose::Create(m_pGraphicDevice, m_pCommandList,
														 wstrRootObjectTag,					// RootObjectTag
														 vecObjectTag[i],					// ObjectTag
														 vecDataFilePath[i],				// DataFilePath
														 vecPos[i],							// Pos
					...
					if (L"QuestButtonCloseMouseOn" == vecObjectTag[i])
					{
						pButtonXMouseOn = static_cast<CQuestButtonClose*>(pChildUI);
						CQuestMgr::Get_Instance()->Set_ButtonCloseClass(static_cast<CQuestButtonClose*>(pChildUI));

					}
					else if (L"QuestButtonCloseMouseClickend" == vecObjectTag[i])
						pButtonXMouseClicked = static_cast<CQuestButtonClose*>(pChildUI);
					else
						pButtonXMouseNormal = static_cast<CQuestButtonClose*>(pChildUI);
...
HRESULT CScene_MainStage::SetUp_UIQuesetMiniCanvas()
{
	{
		wifstream fin{ L"../../Bin/ToolData/2DUIQuestMiniSubCanvas.2DUI" };
		if (fin.fail())
			return E_FAIL;
		...
		while (true)
		{
			...
			{
				Engine::CGameObject* pChildUI = nullptr;
					pChildUI = CQuestComplete::Create(m_pGraphicDevice, m_pCommandList,
													  wstrRootObjectTag,				// RootObjectTag
													  vecObjectTag[i],					// ObjectTag
													  vecDataFilePath[i],				// DataFilePath
													  vecPos[i],						// Pos
	...
	{
		wifstream fin{ L"../../Bin/ToolData/2DUIQuestMiniMainCanvas.2DUI" };
		if (fin.fail())
			return E_FAIL;
...
HRESULT CScene_MainStage::SetUp_UIQuestMessageCanvas()
{
	// MainMenuSettingCanvas
	CQuestMessageButtonClose* pButtonXMouseNormal  = nullptr;
	CQuestMessageButtonClose* pButtonXMouseOn      = nullptr;
	CQuestMessageButtonClose* pButtonXMouseClicked = nullptr;
	{
		wifstream fin{ L"../../Bin/ToolData/2DUIBasicSystemMessage.2DUI" };
		if (fin.fail())
			return E_FAIL;
		...
		while (true)
		{
			...
			// UIRoot 积己.
			Engine::CGameObject* pRootUI = nullptr;
			pRootUI = CQuestMessageCanvas::Create(m_pGraphicDevice, m_pCommandList,
												  wstrRootObjectTag + wstring(L"Quest"),
												  wstrDataFilePath,
												  vPos,
												  vScale,
												  bIsSpriteAnimation,
												  ...
												  vRectScale,
												  UIDepth);
			m_pObjectMgr->Add_GameObject(L"Layer_UI", wstrRootObjectTag + wstring(L"Quest"), pRootUI);
			CQuestMgr::Get_Instance()->Set_QuestMessageCanvasClass(static_cast<CQuestMessageCanvas*>(pRootUI));
			...
			for (_int i = 0; i < iChildUISize; ++i)
			{
				...
				if (L"SystemButtonYes" == vecObjectTag[i] ||
					L"SystemButtonNo" == vecObjectTag[i])
				{
					pChildUI = CQuestMessageButton::Create(m_pGraphicDevice, m_pCommandList,
														   wstrRootObjectTag + wstring(L"Quest"), // RootObjectTag
														   vecObjectTag[i],					// ObjectTag
														   vecDataFilePath[i],				// DataFilePath
														   vecPos[i],						// Pos
														   vecScale[i],						// Scane
						 ...
						 L"SystemButtonX_MouseClicked" == vecObjectTag[i])
				{
					pChildUI = CQuestMessageButtonClose::Create(m_pGraphicDevice, m_pCommandList,
																wstrRootObjectTag + wstring(L"Quest"),	// RootObjectTag
																vecObjectTag[i],				// ObjectTag
																vecDataFilePath[i],				// DataFilePath
																vecPos[i],						// Pos
					...
					if (L"SystemButtonX_MouseOn" == vecObjectTag[i])
						pButtonXMouseOn = static_cast<CQuestMessageButtonClose*>(pChildUI);
					else if (L"SystemButtonX_MouseClicked" == vecObjectTag[i])
						pButtonXMouseClicked = static_cast<CQuestMessageButtonClose*>(pChildUI);
					else
						pButtonXMouseNormal = static_cast<CQuestMessageButtonClose*>(pChildUI);
