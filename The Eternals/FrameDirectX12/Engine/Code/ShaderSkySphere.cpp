...
CShaderSkySphere::CShaderSkySphere(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList)
	: CShader(pGraphicDevice, pCommandList)
{
}

CShaderSkySphere::CShaderSkySphere(const CShaderSkySphere& rhs)
	: CShader(rhs)
{
}

HRESULT CShaderSkySphere::SetUp_ShaderConstantBuffer(const _uint& uiNumSubsetMesh)
{
	m_uiSubsetMeshSize = uiNumSubsetMesh;

	m_pCB_ShaderSkySphere = CUploadBuffer<CB_SHADER_SKYSPHERE>::Create(m_pGraphicDevice);
	NULL_CHECK_RETURN(m_pCB_ShaderSkySphere, E_FAIL);

	return S_OK;
}

HRESULT CShaderSkySphere::Ready_Shader()
{
...
void CShaderSkySphere::Begin_Shader(ID3D12DescriptorHeap* pTexDescriptorHeap, const _uint& iSubMeshIdx)
{
	...
	m_pCommandList->SetGraphicsRootConstantBufferView(4,	// RootParameter Index
													  m_pCB_ShaderSkySphere->Resource()->GetGPUVirtualAddress());


}

HRESULT CShaderSkySphere::Create_RootSignature()
{
	/*__________________________________________________________________________________________________________
	[ SRV를 담는 서술자 테이블을 생성 ]
	____________________________________________________________________________________________________________*/
	...
	CD3DX12_ROOT_SIGNATURE_DESC RootSignatureDesc(5,							// 루트 파라미터 개수.
												  RootParameter,
...
HRESULT CShaderSkySphere::Create_PipelineState()
{
	/*__________________________________________________________________________________________________________
	[ PipelineState 기본 설정 ]
	____________________________________________________________________________________________________________*/
...
vector<D3D12_INPUT_ELEMENT_DESC> CShaderSkySphere::Create_InputLayout(string VS_EntryPoint, string PS_EntryPoint)
{
	vector<D3D12_INPUT_ELEMENT_DESC> vecInputLayout;

	m_pVS_ByteCode = Compile_Shader(L"../../Bin/Shader/ShaderSkySphere.hlsl", nullptr, VS_EntryPoint.c_str(), "vs_5_1");
	m_pPS_ByteCode = Compile_Shader(L"../../Bin/Shader/ShaderSkySphere.hlsl", nullptr, PS_EntryPoint.c_str(), "ps_5_1");
...
D3D12_BLEND_DESC CShaderSkySphere::Create_BlendState(const _bool& bIsBlendEnable, const D3D12_BLEND& SrcBlend, const D3D12_BLEND& DstBlend, const D3D12_BLEND_OP& BlendOp, const D3D12_BLEND& SrcBlendAlpha, const D3D12_BLEND& DstBlendAlpha, const D3D12_BLEND_OP& BlendOpAlpha)
{
...
CComponent* CShaderSkySphere::Clone()
{
	return new CShaderSkySphere(*this);
}

CShader* CShaderSkySphere::Create(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList)
{
	CShaderSkySphere* pInstance = new CShaderSkySphere(pGraphicDevice, pCommandList);
...
void CShaderSkySphere::Free()
{
	CShader::Free();

	Safe_Delete(m_pCB_ShaderSkySphere);
}
