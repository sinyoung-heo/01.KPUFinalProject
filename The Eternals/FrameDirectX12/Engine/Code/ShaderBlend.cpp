...
void CShaderBlend::Begin_Shader(ID3D12DescriptorHeap* pTexDescriptorHeap, const _uint & iIdx)
{
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE SRV_TexEdgeBlurDescriptorHandle(m_pTexDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	SRV_TexEdgeBlurDescriptorHandle.Offset(10, m_uiCBV_SRV_UAV_DescriptorSize);
	m_pCommandList->SetGraphicsRootDescriptorTable(10,		// RootParameter Index. (Tex EdgeBlur)
		SRV_TexEdgeBlurDescriptorHandle);

	//Cbuffer
	m_pCommandList->SetGraphicsRootConstantBufferView(11,	// RootParameter Index
...
HRESULT CShaderBlend::Create_RootSignature()
{
	...
	[ SRV를 담는 서술자 테이블을 생성 ]
	____________________________________________________________________________________________________________*/
	CD3DX12_DESCRIPTOR_RANGE SRV_Table[11];
	...
	// Texture - Distortion
	SRV_Table[6].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,	// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		6,								// 셰이더 인수들의 기준 레지스터 번호. (register t6)
		0);								// 레지스터 공간.

		// Texture - average color
	SRV_Table[7].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,	// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		7,								// 셰이더 인수들의 기준 레지스터 번호. (register t7)
		0);								// 레지스터 공간.
		// Texture - average color
	SRV_Table[8].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,	// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		8,								// 셰이더 인수들의 기준 레지스터 번호. (register t8)
		0);								// 레지스터 공간.

	SRV_Table[9].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,	// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		9,								// 셰이더 인수들의 기준 레지스터 번호. (register t9)
		0);								// 레지스터 공간.
	SRV_Table[10].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,	// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		10,								// 셰이더 인수들의 기준 레지스터 번호. (register t10)
		0);								// 레지스터 공간.
	/*__________________________________________________________________________________________________________
	...
	RootParameter[11].InitAsConstantBufferView(0); //b0

	auto StaticSamplers = Get_StaticSamplers();
	CD3DX12_ROOT_SIGNATURE_DESC RootSignatureDesc(12,							// 루트 파라미터 개수.(SRV 10 : 총10개)
												  RootParameter,
...
HRESULT CShaderBlend::Create_PipelineState()
{
	...
	PipelineStateDesc.DepthStencilState		= CShader::Create_DepthStencilState(false);	// Blend할 때 Depth는 false
