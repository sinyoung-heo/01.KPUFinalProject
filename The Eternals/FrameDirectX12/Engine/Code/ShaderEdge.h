...
class ENGINE_DLL CShaderEdge final : public CShader
{
private:
	explicit CShaderEdge(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	explicit CShaderEdge(const CShaderEdge& rhs);
	virtual ~CShaderEdge() = default;
...
public:
	...
	static CShaderEdge*	Create(ID3D12Device* pGraphicDebice, ID3D12GraphicsCommandList* pCommandList);
private:
	virtual void			Free();
