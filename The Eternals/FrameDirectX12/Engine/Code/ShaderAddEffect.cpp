...
void CShaderAddEffect::Begin_Shader(ID3D12DescriptorHeap* pTexDescriptorHeap, const _uint & iIdx)
{
	...
	CD3DX12_GPU_DESCRIPTOR_HANDLE SRV_DynamicMeshEffectDescriptorHandle(m_pTexDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
	SRV_DynamicMeshEffectDescriptorHandle.Offset(6, m_uiCBV_SRV_UAV_DescriptorSize);
	m_pCommandList->SetGraphicsRootDescriptorTable(6,
		SRV_DynamicMeshEffectDescriptorHandle);
}
...
HRESULT CShaderAddEffect::Create_RootSignature()
{
	...
	SRV_Table[6].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV,// 서술자의 종류 - Shader Resource View.
		1,								// 서술자의 개수 - Texture2D의 개수.
		6,								// 셰이더 인수들의 기준 레지스터 번호. (register t4)
		0);								// 레지스터 공간.
	/*__________________________________________________________________________________________________________
	...
	RootParameter[6].InitAsDescriptorTable(1, &SRV_Table[6], D3D12_SHADER_VISIBILITY_PIXEL);	// t4

	auto StaticSamplers = Get_StaticSamplers();
	CD3DX12_ROOT_SIGNATURE_DESC RootSignatureDesc(7,							// 루트 파라미터 개수.(SRV 5: 총 5개)
												  RootParameter,
