...
CShaderMeshTerrain::CShaderMeshTerrain(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
	: CShader(pGraphicDevice, pCommandList)
{
}

CShaderMeshTerrain::CShaderMeshTerrain(const CShaderMeshTerrain & rhs)
	: CShader(rhs)
{
...
HRESULT CShaderMeshTerrain::Ready_Shader()
{
...
void CShaderMeshTerrain::Begin_Shader(ID3D12DescriptorHeap* pTexDescriptorHeap, const _uint& iSubMeshIdx)
{
...
void CShaderMeshTerrain::Begin_Shader(ID3D12GraphicsCommandList * pCommandList,
...
HRESULT CShaderMeshTerrain::SetUp_ShaderConstantBuffer(const _uint& uiNumSubsetMesh, const _uint& iAfterImgSize)
{
...
HRESULT CShaderMeshTerrain::Create_RootSignature()
{
	/*__________________________________________________________________________________________________________
	[ SRV를 담는 서술자 테이블을 생성 ]
	____________________________________________________________________________________________________________*/
	...
	RootParameter[6].InitAsConstantBufferView(1);		// register b0.
...
HRESULT CShaderMeshTerrain::Create_PipelineState()
{
	/*__________________________________________________________________________________________________________
	[ PipelineState 기본 설정 ]
	____________________________________________________________________________________________________________*/
	...
	PipelineStateDesc.SampleDesc.Quality = CGraphicDevice::Get_Instance()->Get_MSAA4X_Enable() ? (CGraphicDevice::Get_Instance()->Get_MSAA4X_QualityLevels() - 1) : 0;
	PipelineStateDesc.DSVFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;
	vecInputLayout = Create_InputLayout("VS_TERRAIN_MAIN", "PS_MAGICCIRCLE");
	PipelineStateDesc.InputLayout = { vecInputLayout.data(), (_uint)vecInputLayout.size() };
	PipelineStateDesc.VS = { reinterpret_cast<BYTE*>(m_pVS_ByteCode->GetBufferPointer()), m_pVS_ByteCode->GetBufferSize() };
	PipelineStateDesc.PS = { reinterpret_cast<BYTE*>(m_pPS_ByteCode->GetBufferPointer()), m_pPS_ByteCode->GetBufferSize() };
	PipelineStateDesc.BlendState = Create_BlendState(false, D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_BLEND_OP_ADD, D3D12_BLEND_ONE,
...
vector<D3D12_INPUT_ELEMENT_DESC> CShaderMeshTerrain::Create_InputLayout(string VS_EntryPoint,
																  string PS_EntryPoint)
{
	vector<D3D12_INPUT_ELEMENT_DESC> vecInputLayout;

	m_pVS_ByteCode = Compile_Shader(L"../../Bin/Shader/ShaderMeshTerrain.hlsl", nullptr, VS_EntryPoint.c_str(), "vs_5_1");
	m_pPS_ByteCode = Compile_Shader(L"../../Bin/Shader/ShaderMeshTerrain.hlsl", nullptr, PS_EntryPoint.c_str(), "ps_5_1");
...
D3D12_BLEND_DESC CShaderMeshTerrain::Create_BlendState(const _bool& bIsBlendEnable,
...
CComponent * CShaderMeshTerrain::Clone()
{
	return new CShaderMeshTerrain(*this);
}

CShader* CShaderMeshTerrain::Create(ID3D12Device * pGraphicDevice, ID3D12GraphicsCommandList * pCommandList)
{
	CShaderMeshTerrain* pInstance = new CShaderMeshTerrain(pGraphicDevice, pCommandList);
