...
HRESULT CRenderTarget::Ready_RenderTarget(const _uint& uiTargetCnt)
{
	...
	// Target Rendering
	m_pBufferCom = static_cast<CScreenTex*>(CComponentMgr::Get_Instance()->Clone_Component(L"Prototype_ScreenTex", COMPONENTID::ID_STATIC));
	NULL_CHECK_RETURN(m_pBufferCom, E_FAIL);

	m_pShaderCom = static_cast<CShaderTexture*>(CComponentMgr::Get_Instance()->Clone_Component(L"Prototype_ShaderTexture", COMPONENTID::ID_STATIC));
...
void CRenderTarget::SetUp_ShaderConstantBuffer()
{
	m_pShaderCom->SetUp_ShaderConstantBuffer((_uint)(m_vecTargetTexture.size()));
}

HRESULT CRenderTarget::SetUp_DefaultSetting()
{
...
HRESULT CRenderTarget::SetUp_LightSetting()
{
	/*__________________________________________________________________________________________________________
	[ Descripotr ]
	____________________________________________________________________________________________________________*/
	...
	for (_uint i = 0; i < m_uiTargetCnt; ++i)
	{
		...
		m_pGraphicDevice->CreateRenderTargetView(m_vecTargetTexture[i].Get(), &RTV_Desc,
												 CD3DX12_CPU_DESCRIPTOR_HANDLE(m_pRTV_Heap->GetCPUDescriptorHandleForHeapStart(),
												 							   i,
												 							   m_uiRTV_DescriptorSize));
	}
...
HRESULT CRenderTarget::SetUp_ShadowDepthSetting()
{
	// Descriptor Size
	m_uiRTV_DescriptorSize = CGraphicDevice::Get_Instance()->Get_RTV_DescriptorSize();
	m_uiDSV_DescriptorSize = CGraphicDevice::Get_Instance()->Get_DSV_DescriptorSize();
	...
	DepthStencilDesc.Width				= (_uint)m_vecViewPort[0].Width;
	DepthStencilDesc.Height				= (_uint)m_vecViewPort[0].Height;
	...
	FAILED_CHECK_RETURN(m_pGraphicDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
																  ...
																  &optClear,
																  IID_PPV_ARGS(&m_vecDepthStencilBuffer[0])),
																  E_FAIL);
	
	/*__________________________________________________________________________________________________________
	[ 20.06.12 ] MultiThreadRendering
	____________________________________________________________________________________________________________*/
	for (_uint i = 0; i < m_uiTargetCnt; ++i)
	{
		// RenderTarget View
		...
		RTV_Desc.Texture2D.PlaneSlice	= 0;
		RTV_Desc.ViewDimension			= D3D12_RTV_DIMENSION_TEXTURE2D;
		RTV_Desc.Format					= m_vecTargetFormat[0];
	...
	// DepthStencil View
	m_pGraphicDevice->CreateDepthStencilView(m_vecDepthStencilBuffer[0].Get(),
											 nullptr,
...
HRESULT CRenderTarget::SetUp_OnGraphicDevice()
{
	/*__________________________________________________________________________________________________________
...
HRESULT CRenderTarget::SetUp_LightOnGraphicDevice()
{
	/*__________________________________________________________________________________________________________
	- 현재의 BackBuffer RenderTarget을 RENDER_TARGET상태에서 PRESENT 상태로 잠시 내려놓는다.
	____________________________________________________________________________________________________________*/
...
HRESULT CRenderTarget::SetUp_ShadowDepthOnGraphicDevice()
{
	/*__________________________________________________________________________________________________________
	- 현재의 BackBuffer RenderTarget을 RENDER_TARGET상태에서 PRESENT 상태로 잠시 내려놓는다.
	____________________________________________________________________________________________________________*/
	...
	for (_uint i = 0; i < m_uiTargetCnt; ++i)
	{
		...
		m_pCommandList->ResourceBarrier(1,
			&CD3DX12_RESOURCE_BARRIER::Transition(m_vecDepthStencilBuffer[i].Get(),
												  D3D12_RESOURCE_STATE_COMMON,
...
HRESULT CRenderTarget::Release_OnGraphicDevice()
{
	/*__________________________________________________________________________________________________________
...
HRESULT CRenderTarget::Release_ShadowDepthOnGraphicDevice()
{
	for (_uint i = 0; i < m_uiTargetCnt; ++i)
	{
		m_pCommandList->ClearDepthStencilView(CD3DX12_CPU_DESCRIPTOR_HANDLE(m_pDSV_Heap->GetCPUDescriptorHandleForHeapStart(),
...
HRESULT CRenderTarget::Begin_RenderTargetOnContext(ID3D12GraphicsCommandList * pCommandList, const THREADID& eID)
{
	if (THREADID::SHADOW == eID)
	{
		...
		pCommandList->ResourceBarrier(1,
			&CD3DX12_RESOURCE_BARRIER::Transition(m_vecDepthStencilBuffer[0].Get(),
												  D3D12_RESOURCE_STATE_COMMON,
												  D3D12_RESOURCE_STATE_DEPTH_WRITE));
