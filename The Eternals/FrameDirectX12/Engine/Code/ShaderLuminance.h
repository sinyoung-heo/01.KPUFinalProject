...
class ENGINE_DLL CShaderLuminance final : public CShader
{
private:
	explicit CShaderLuminance(ID3D12Device* pGraphicDevice, ID3D12GraphicsCommandList* pCommandList);
	explicit CShaderLuminance(const CShaderLuminance& rhs);
	virtual ~CShaderLuminance() = default;
...
public:
	...
	static CShaderLuminance*	Create(ID3D12Device* pGraphicDebice, ID3D12GraphicsCommandList* pCommandList);
private:
	virtual void			Free();
